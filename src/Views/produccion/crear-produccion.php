...
$(document).ready(function() {
    ...
    // Cargar receta al seleccionar producto - VERSIÓN MODIFICADA
    $('#id_producto').change(function() {
        var idProducto = $(this).val();
        console.log('🔄 Producto seleccionado:', idProducto);
        
        // Limpiar campo cantidad cuando se cambia de producto
        $('#cantidad_planificada').val('');
        ...
        if (idProducto) {
            ...
            // Si no hay producto seleccionado, deshabilitar campo
            $('#cantidad_planificada').prop('disabled', true).val('');
            $('#btnCrearOrden').hide();
        }
    });
    ...
    $('#cantidad_planificada').on('input', function() {
        ...
        debounceTimeout = setTimeout(function() {
            if ($('#id_producto').val() && $('#cantidad_planificada').val() > 0) {
                verificarStock();
            }
        }, 800); // Espera 800ms
    });
...
function validarFormulario() {
    ...
    // El botón de verificar ya no existe, así que no lo manipulamos.
   ...
   function cargarProductos() {
    ...
    // DESHABILITAR campo cantidad y OCULTAR botón inicialmente
    $('#cantidad_planificada').prop('disabled', true).val('');
    $('#btnCrearOrden').hide();
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.status === 200 && response.data.length > 0) {
                ...
                // HABILITAR campo cantidad y MOSTRAR botón si tiene receta
                $('#cantidad_planificada').prop('disabled', false);
                $('#btnCrearOrden').show();
                console.log('✅ Receta cargada exitosamente - Campo HABILITADO, Botón MOSTRADO');
            } else {
                ...
                // DESHABILITAR campo cantidad y OCULTAR botón si no tiene receta
                $('#cantidad_planificada').prop('disabled', true).val('');
                $('#btnCrearOrden').hide();
                console.warn('⚠️ No se encontró receta para el producto - Campo DESHABILITADO, Botón OCULTO');
            }
        },
        error: function(xhr, status, error) {
            ...
            // Por seguridad, deshabilitar campo y ocultar botón si hay error
            $('#cantidad_planificada').prop('disabled', true).val('');
            $('#btnCrearOrden').hide();
        }
    });
    ...
    // No deshabilitamos el botón porque ya no existe.
